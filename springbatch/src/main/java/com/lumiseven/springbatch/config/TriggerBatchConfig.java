...
public class TriggerBatchConfig {
	...
	/*
	 * 修改返回值类型为FlatFileItemReader
	 * 配置itemReader
	 */
	@Bean
	@StepScope
	public FlatFileItemReader<Member> reader(@Value("#{jobParameters['input.file.name']}") String pathToFile) throws Exception {
		FlatFileItemReader<Member> reader = new FlatFileItemReader<Member>();
		reader.setResource(new ClassPathResource(pathToFile));//设置data文件路径
